/**
 * Pure copy data and mapping functions for the EscrowSigningModal.
 *
 * No JSX. No React imports. Safe to import from tests directly.
 */
...
/** Copy override shape accepted by the modal and passed to getStepInfo. */
...
// ─── Step / operation copy tables ─────────────────────────────────────────────

/**
 * Human framing for one on-chain step, keyed by (operation, step).
 * Used when a multi-step flow needs per-step messaging.
 */
...
/** Fallback copy when no per-step entry exists for the given operation. */
export const OPERATION_FALLBACK_INFO: Record<EscrowOperation, StepDetail> = {
  ...
  release: {
    ...
    actionExplanation: "You are authorizing the release of escrowed payment.",
  },
  refund: {
    position: "Escrow refund",
    confirmedMessage: "Refund request submitted on-chain.",
    actionTitle: "Request Escrow Refund",
    actionExplanation: "You are submitting an on-chain refund request.",
  },
  dispute: {
    position: "Escrow dispute",
    confirmedMessage: "Dispute recorded on-chain.",
    actionTitle: "Open Escrow Dispute",
    actionExplanation: "You are opening a dispute on the Stellar blockchain.",
  },
};
...
/**
 * Resolves the copy block for the current operation/step combination.
 * Returns the `copy` override verbatim when provided.
 */
export function getStepInfo(
  operation: EscrowOperation | undefined,
  step: EscrowStepName | null | undefined,
  copyOverride: EscrowSigningCopyOverride | undefined
): StepDetail | null {
  if (copyOverride) {
...
export function errorCopy(error: EscrowSigningError): {
  title: string;
  message: string;
  actionHint: string;
} {
...
/** Modal header title for each signing state. */
...
/** Returns a shortened display form of a transaction hash, e.g. `a1b2c3d4…e5f6a7b8`. */
